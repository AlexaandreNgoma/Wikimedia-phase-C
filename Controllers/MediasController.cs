...
public class MediasController : Controller
{
            ...
            if ((bool)Session["SortAscending"])
            {
                switch ((MediaSortBy)Session["MediaSortBy"])
                {
                    ...
                    case MediaSortBy.Likes:
                        result = result.OrderBy(c => DB.Likes.GetByMedia(c.Id).Count); break;
                }
            }
            else
            {
                switch ((MediaSortBy)Session["MediaSortBy"])
                {
                    ...
                    case MediaSortBy.Likes:
                        result = result.OrderByDescending(c => DB.Likes.GetByMedia(c.Id).Count); break;
                }
            }
    ...
    public ActionResult ToggleLike(int mediaId)
    {
        ...
        var existing = DB.Likes.GetUserLike(mediaId, user.Id);
        if (existing != null)
            DB.Likes.Delete(existing.Id);
        else
            DB.Likes.Add(new Models.Likes { MediaId = mediaId, UserId = user.Id });
    ...
    // ----- Action étape C -----
    public ActionResult GetMediasCreatorsList(bool forceRefresh = false)
    {
        try
        {
            InitSessionVariables();
    ...
    // ----- Action étape C -----
    public ActionResult SetSearchCreator(string value)
    {
        ResetMediasPaging();
    ...
    // ----- Action étape C -----
    public ActionResult GetLikesPanel(int mediaId)
    {
        var user = Models.User.ConnectedUser;
        var likes = DB.Likes.GetByMedia(mediaId);
        var likers = likes.Select(l => DB.Users.Get(l.UserId)).Where(u => u != null).ToList();
        ViewBag.MediaId = mediaId;
        ViewBag.LikesCount = likes.Count;
        ViewBag.UserHasLiked = likes.Any(l => l.UserId == user.Id);
        ViewBag.LikerNames = string.Join(", ", likers.Select(u => u.Name));
        return PartialView();
    }
